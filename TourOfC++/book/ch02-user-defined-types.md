...
`Vector`֊ի այս առաջին տարբերակը բաղկացած է `int` և `double*` տարրերից։
...
`Vector` տիպի փոփոխականը կարող է սահմանվել հետևյալ կերպ․
   ...
   Vector v;
   ````
Սակայն, այն այդ տեսքով շատ գործածելի չէ, քանի որ `v`֊ի `elem` ցուցիչը ոչ մի բանի վրա ցույց չի տալիս։ Օգտակարության համար, մենք պետք է  տանք `v`-ին տարրեր, որոնք կարող ենք ցույց տալ։ Օրինակ, `Vector`֊ը կարող ենք կառուցել հետևյալ ձևով․
...
Եթե չեք ցանկանում խստորեն սահմանափակել թվարկման անունները և ցանկանում եք, որ անուններն ունենան `ամբողջ` արժեքներ (), կարող եք `enum class`֊ից հեռացնել  `class` բառը և կստանաք «սովորական» `թվարկում` (`enum`): The enumerators from a ‘‘plain’’ enum are enteredColor into the same scope as the name of their enum and implicitly converts to their integer value. For example:

````c++
   enum Color {red, green, blue};
   int col = green;
```` 
Այստեղ `col`֊ը ստանում է `1` արժեքը։ Լռությամբ թվարկվող անունների ամբողջ արժեքները սկսում են `0`֊ից և ավելանում են մեկով յուրաքանչյուր թվարկվող անվան հետ։ «Հասարակ» `թվարկումները` եղել են C++-ում (և C-ում) սկզբնական օրերից, so even though they are less well behaved, դրանք ընթացիկ կոդերում էլ գործածելի են։
